...
import { getClientReplicateModel } from '../../lib/ai/get-replicate-model';
...
export async function generateContent() {
  ...
  for (const client of clients) {
    try {
      ...
      // Determine the last scheduled date to start scheduling after
      let lastScheduledDate = new Date();
      if (existingPosts && existingPosts.length > 0) {
        // Find the latest scheduled date
        const latestPost = existingPosts.reduce((latest, post) => {
          const postDate = new Date(post.scheduled_at);
          return postDate > latest ? postDate : latest;
        }, new Date());
        lastScheduledDate = latestPost;
      }
      ...
      for (let i = 0; i < postsNeeded; i++) {
        ...
        let content = await generateAI({
          ...
          brandAnalysis: brandAssets?.ai_analysis as any, // Pass AI brand analysis
        });
        ...
        try {
          // Get client's preferred image model
          const clientModel = await getClientReplicateModel(client.id);
          ...
          const imageResult = await generateImage({
            ...
            model: clientModel, // Use client's preferred model
          });
          imageUrl = imageResult.imageUrl;
          imageModel = imageResult.model;

          // Generate Video for every 4th post (25% chance) or if explicitly requested
          // Only if we have a valid image to start from
          if (imageUrl && (i % 4 === 0)) {
            try {
              const { generateVideoFromImage } = await import('../../lib/ai/video-generation');
              console.log(`🎥 Generating video for ${client.name}...`);
              const videoResult = await generateVideoFromImage(imageUrl);
              videoUrl = videoResult.videoUrl;
              console.log(`✅ Video generated: ${videoUrl}`);
            } catch (vidError: any) {
              console.warn(`⚠️ Video generation failed: ${vidError?.message}`);
            }
          }
        ...
        // Pass lastScheduledDate to ensure we schedule AFTER the previous post
        ...
        // Update lastScheduledDate for the next iteration
        lastScheduledDate = nextPostDate;
        ...
        const { data: postData, error } = await supabase
          ...
          .insert({
            ...
            validation_result: {
              ...
              videoUrl: videoUrl || null // Store video URL here since we can't add column
            },
